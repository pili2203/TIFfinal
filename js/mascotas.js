...
createApp({
  data() {
     ...
     */
    return {
      mascotas: [], // Almacena los productos obtenidos del backend
      // url:'http://localhost:5000/productos', // URL local
      url: "https://pilipi1.pythonanywhere.com/mascotas", // URL del backend donde se encuentran los productos
      error: false,
      cargando: true,
  ...
  methods: {
    ...
    grabar() {
      ...
      var options = {
        body: JSON.stringify(mascota), // Convertir el objeto a una cadena JSON
      ...
      fetch(this.url, options)
        .then(function () {
          ...
          window.location.href = "mascotas.html"; // Redirigir a la página de productos
        })
        .catch((err) => {
          console.error(err);
